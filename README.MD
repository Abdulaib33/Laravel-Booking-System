
---

# 2. Laravel Appointment Booking System — polished GitHub README


# Laravel Appointment Booking System

![Laravel](https://img.shields.io/badge/Laravel-12-red)
![PHP](https://img.shields.io/badge/PHP-8.2+-777BB4)
![MySQL](https://img.shields.io/badge/MySQL-Database-4479A1)
![Blade](https://img.shields.io/badge/Blade-Templating-orange)
![Tailwind CSS](https://img.shields.io/badge/TailwindCSS-UI-38B2AC)
![Status](https://img.shields.io/badge/Status-Portfolio%20Project-success)

A booking management application built with Laravel.

Users can submit appointment requests, while administrators manage services and booking statuses through an admin dashboard.

## Live Demo

- Demo: `Coming soon`
- Admin: `Coming soon`

## Features

- Authentication
- Appointment booking form
- Services management
- Admin booking dashboard
- Booking status workflow
- Status filtering in admin panel
- Validation and friendly error messages
- Double-booking prevention
- Eloquent relationships between users, services, and bookings

## Screenshots

### Booking Form
![Booking Form](./screenshots/booking-form.png)

### Admin Bookings Dashboard
![Admin Bookings](./screenshots/admin-bookings.png)

### Services Management
![Services](./screenshots/services.png)

## Tech Stack

**Backend**
- PHP
- Laravel

**Frontend**
- Blade
- Tailwind CSS

**Database**
- MySQL

**Tools**
- Git
- GitHub
- Linux
- Apache

## Installation

```bash
git clone https://github.com/yourusername/laravel-booking-system.git
cd laravel-booking-system
composer install
npm install
cp .env.example .env
php artisan key:generate
```



Business Logic Highlights

Prevents duplicate time slots for the same service

Uses validation plus database constraints for booking protection

Supports booking lifecycle:

pending

approved

rejected

completed

Restricts admin area with role-based middleware

Project Highlights

Practical real-world booking flow

Stronger than basic CRUD because it includes business rules

Uses Eloquent relationships and admin workflows

Shows backend thinking beyond UI only

Roadmap

Add cancellation by user

Add time-slot generation

Add email notifications for booking status changes

Deploy live version on Railway

Author

Built as a portfolio project to demonstrate Laravel business logic, booking workflows, and admin dashboard development.